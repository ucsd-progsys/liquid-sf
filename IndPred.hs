...
{-
Need to define in SMTLIB

   ctor IsInsP : a -> List a -> List a -> IsIns a

Need to write assumes:

   Here :: m:a
        -> ms:List a
        -> { v : IsIns a | v = IsInsP m ms (Cons m ms) }

   There :: m:a -> n:a -> ns:List a
         -> mns:{List a | isIns m ns mns }
         -> {v : IsIns a | v = IsInsP m (Cons n ns) (Cons n mns)}

G := p :: {IsIns a | p = IsIns x ys xys}

     case p of
       Here a bs -> ... G'

G' := G; {p = IsInsP a bs (Cons a bs)}

Now, as

1. `(IsInsP a bs (Cons a bs)) = (IsInsP x ys xys)`

2. `IsInsP` is an SMT-ctor (like Cons),

we get the "inversion"

3. a = x, bs = ys, (Cons a bs) = xys
